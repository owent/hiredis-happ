...
| Root `CLAUDE.md` imports `@AGENTS.md`. | Verified for Claude Code | Claude Code reads `CLAUDE.md`; import keeps the source of truth in one place. |
...
| `.github/copilot-instructions.md` remains a thin shim. | Verified for VS Code/Copilot | VS Code supports this file, but duplicating the full rule set increases context bloat. |
| `.github/copilot/skills/*.md` remains only as legacy pointers. | Compatibility shim | This path was already present in the repo, but standard project skill paths are `.github/skills/`, `.claude/skills/`, and `.agents/skills/`. |
...
| VS Code/Copilot customizations | <https://code.visualstudio.com/docs/copilot/customization/custom-instructions>, <https://code.visualstudio.com/docs/copilot/customization/agent-skills>, <https://code.visualstudio.com/docs/copilot/customization/custom-agents>, <https://code.visualstudio.com/docs/copilot/customization/prompt-files> | VS Code supports repository instructions including `.github/copilot-instructions.md`, `AGENTS.md`, and `CLAUDE.md`; project skill locations include `.github/skills/`, `.claude/skills/`, and `.agents/skills/`; prompt files and custom agents are separate primitives. |
| Claude Code | <https://code.claude.com/docs/en/memory>, <https://code.claude.com/docs/en/skills>, <https://www.anthropic.com/engineering/equipping-agents-for-the-real-world-with-agent-skills> | Claude Code reads `CLAUDE.md`; a thin `@AGENTS.md` import is appropriate; project skills follow the Agent Skills model with Claude-specific extensions; keep memory files concise and move procedures to skills. |
