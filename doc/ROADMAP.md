...
1. 使用 docker compose 或脚本启动 3 master + 3 replica 测试集群。（已由 `test/redis/redis-fixture.sh` 覆盖：Linux 上默认通过 Docker 容器启动 6 节点集群，macOS 默认使用 Homebrew 预编译 `redis` 包，其余环境回退源码构建；`HIREDIS_HAPP_TEST_REDIS_PROVIDER` 可显式指定。）
2. 增加以下测试：
