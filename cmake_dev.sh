...
while getopts "ab:c:d:e:hlr:tus-" OPTION; do
    case $OPTION in
        ...
        ;;
        u)
            CMAKE_OPTIONS="$CMAKE_OPTIONS -DPROJECT_ENABLE_UNITTEST=YES -DPROJECT_HIREDIS_HAPP_ENABLE_UNITTEST";
        ;;
        s)
            CMAKE_OPTIONS="$CMAKE_OPTIONS -DPROJECT_ENABLE_SAMPLE=YES -DPROJECT_HIREDIS_HAPP_ENABLE_SAMPLE=ON";
        ;;
        -)
            break;
        ;;
